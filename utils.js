...
let globalProxy = process.env.FB_PROXY || null;
...
function saveCookiesToJar(jar, setCookieHeaders, requestUrl) {
...
function buildAxiosConfig(method, requestUrl, headers, jar, extraConfig) {
        const config = {
                method,
                url: requestUrl,
        ...
        if (globalProxy) {
                ...
                config.httpsAgent = new HttpsProxyAgent(globalProxy);
                config.httpAgent = new HttpsProxyAgent(globalProxy);
        }
        return config;
}

function wrapResponse(axiosRes, originalUrl, originalMethod, requestHeaders, requestBody) {
        const parsedUrl = new url.URL(originalUrl);
        const body = typeof axiosRes.data === "string"
                ? axiosRes.data
                : (Buffer.isBuffer(axiosRes.data) ? axiosRes.data.toString("utf8") : JSON.stringify(axiosRes.data));
        return {
                body,
...
function get(requestUrl, jar, qs, options, ctx, customHeader) {
        ...
        const headers = getHeaders(requestUrl, options, ctx, customHeader);
        const config = buildAxiosConfig("GET", requestUrl, headers, jar, { params: qs, responseType: "arraybuffer" });

        return axios(config).then(function (res) {
                saveCookiesToJar(jar, res.headers["set-cookie"], requestUrl);
                const body = res.data ? res.data.toString("utf8") : "";
                return wrapResponse({ ...res, data: body }, requestUrl, "GET", config.headers, null);
        });
}

function post(requestUrl, jar, form, options, ctx, customHeader) {
        const headers = getHeaders(requestUrl, options, ctx, customHeader);
        headers["Content-Type"] = "application/x-www-form-urlencoded";
        const encoded = querystring.stringify(form);
        const config = buildAxiosConfig("POST", requestUrl, headers, jar, { data: encoded, responseType: "arraybuffer" });

        return axios(config).then(function (res) {
                saveCookiesToJar(jar, res.headers["set-cookie"], requestUrl);
                const body = res.data ? res.data.toString("utf8") : "";
                return wrapResponse({ ...res, data: body }, requestUrl, "POST", config.headers, form);
        });
}

function postFormData(requestUrl, jar, form, qs, options, ctx) {
        const fd = new FormData();
        for (const key in form) {
                if (form.hasOwnProperty(key)) {
                        const val = form[key];
                        if (val && typeof val === "object" && val.value !== undefined) {
                                fd.append(key, val.value, val.options || {});
                        } else {
                                fd.append(key, val);
                        }
                }
        }
        const headers = Object.assign(getHeaders(requestUrl, options, ctx), fd.getHeaders());
        const config = buildAxiosConfig("POST", requestUrl, headers, jar, {
                data: fd,
                params: qs,
...
function makeDefaults(html, userID, ctx) {
        ...
        // } catch(e) {
        //   log.warn("fca-azadx69x", "Couldn't parse SiteData. Won't have access to some variables.");
        //   siteData = {};
        // }
...
function parseAndCheckLogin(ctx, defaultFuncs, retryCount, sourceCall) {
        ...
        return function (data) {
                return tryPromise(function () {
                        log.verbose("fca-azadx69x", data.body);
...
function getAppState(jar) {
        return jar
                ...
                .concat(jar.getCookiesSync("https://facebook.com"))
